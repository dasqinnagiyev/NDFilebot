...
async def send_msg(user_id, message):
    try:
        ...
        return 400, f"{user_id} : user id tapılmadı\n"
...
async def main_broadcast_handler(m, db):
    ...
    out = await m.reply_text(
        text=f"Toplu mesaj yayımı başladı! Bütün istifadəçilərə göndərildikdən sonra sizə log faylı ilə xəbər verəcəm."
    )
    ...
    async with aiofiles.open('dasqin-nagiyev.txt', 'w') as broadcast_log_file:
